...
#define SEM_MANEJANDO "/sem_manejando"
...
sem_t *sem = NULL;
// Semáforo usado para no recibir dos señales donde una corte a la otra
sem_t *manejando = NULL;
...
void child_term_handler(int sig) {
    // Para no tener que cerrar el archivo "a lo bruto", dejamos que se ejecute y luego salimos del proceso. 
    if(sig == SIGTERM)
    {
        sem_close(sem);
        if(file != NULL) {
...
int main(int argc, char** argv){
    ...
    // Otro semáforo sin GOTO, repetimos código (el compilador lo optimiza pero visualmente queda más raro)
    if ((manejando = sem_open(SEM_MANEJANDO, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 1)) == SEM_FAILED
        && errno != 17) { // 17 significa que el semáforo ya existe 
		perror("sem_open");
		exit(EXIT_FAILURE);
	}
    else {
        sem_unlink(SEM_MANEJANDO);
        if ((manejando = sem_open(SEM_MANEJANDO, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 1)) == SEM_FAILED) {
            perror("sem_open");
		    exit(EXIT_FAILURE);
        }
    ...
    for (i = 0; i < N; i++) {
        ...
        if (pid[i] == 0) {
            ...
            // sem_post(manejando); // sem_post se hace en el padre al terminar de manejar la señal
