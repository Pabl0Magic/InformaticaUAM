...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_form_invalid_renewal_date_future(self):
        ...
        invalid_date_in_future = (datetime.date.today()) + (datetime.
                                                            timedelta(weeks=5))
        response = self.client.post(reverse('renew-book-librarian',
                                            kwargs={'pk': self.
                                                    test_bookinstance1.pk}),
...
class AuthorCreateViewTest(TestCase):
    ...
    def test_redirects_to_detail_view_on_success(self):
        ...
        pk = len(response.url)-1
        self.assertRedirects(
            response,
            reverse('author-detail', kwargs={'pk': response.url[pk]})
        )

    # We added default birth date as well
    def test_form_date_of_birth_initially_set_to_expected_date(self):
        self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)

        initial_date = '09/09/1951'
        self.assertEqual(
            response.context['form'].initial['date_of_birth'],
            initial_date
        )
