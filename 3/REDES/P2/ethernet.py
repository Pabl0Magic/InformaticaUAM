...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if (dest!=broadcastAddr and dest!=macAddress):
        return
    
    ethertypeUnpack = struct.unpack('h', ethertype)
    
    if ethertypeUnpack in upperProtos:
        upperProtos[ethertypeUnpack](us, header, data[14:], org)      
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    ethertypeUnpack = struct.unpack('h', ethertype)
    upperProtos[ethertypeUnpack] = callback_func
...
def stopEthernetLevel()->int:
    ...
    '''
    if((levelInitialized == False) or (handle == None)):
        return -1    
...
def sendEthernetFrame(data:bytes,len:int,etherType:int,dstMac:bytes) -> int:
    ...
    trama.extend(dstMac)
    trama.extend(macAddress)
    trama.extend(bytes(etherType))
    trama.extend(data)
    #Hay que añadir los 14 bytes de la cabecera comun
    tramalen = len + 14

    if tramalen > ETH_FRAME_MAX:
        return -1
    
    if tramalen < ETH_FRAME_MIN: 
        trama.extend(bytes([0])*(ETH_FRAME_MIN - tramalen))
        tramalen = ETH_FRAME_MIN

    ret = pcap_inject(handle, bytes(trama), tramalen)
    if ret == -1:
        return -1
